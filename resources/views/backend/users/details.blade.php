            ...
            <div class="row">
                ...
                <div class="custom-xxxl-3 custom-xxl-4 col-md-6 col-sm-6 col-12 mb-4">
                    <div class="card card-statistic-1 style-two mb-0">
                        ...
                        <div class="card-wrap">
                            ...
                            <div class="card-body">
                                {{ $user->refferedBy?->username ? $user->refferedBy?->username : 'N/A' }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="custom-xxxl-3 custom-xxl-4 col-md-6 col-sm-6 col-12 mb-4">
                    <div class="card card-statistic-1 style-two mb-0">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{ __('Total Trading Profit') }}</h4>
                            </div>
                            <div class="card-body">
                                {{ number_format($tradingProfit,2) . ' ' . $general->site_currency }}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="custom-xxxl-3 custom-xxl-4 col-md-6 col-sm-6 col-12 mb-4">
                    <div class="card card-statistic-1 style-two mb-0">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{ __('Total Trading Lose') }}</h4>
                            </div>
                            <div class="card-body">
                                {{ number_format($tradingLose,2) . ' ' . $general->site_currency }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {{-- <div class="custom-xxxl-3 custom-xxl-4 col-md-6 col-sm-6 col-12 mb-4">
                    <div class="card card-statistic-1 style-two mb-0">
                        <div class="card-icon rounded-circle gr-bg-8">
                        ...
                        <div class="custom-xxl-3 col-xl-4 col-lg-5">
                            ...
                            <form action="{{ route('admin.user.balance.update', $user->id) }}" method="post">
                                ...
                                <div class="input-group mb-3">
                                    ...
                                    <button class="btn btn-outline-success px-4" type="submit" id="button-addon2"> <i class="fa fa-plus"></i> {{ __('Add Balance') }}</button>
                                </div>
                            </form>
                            <form action="{{ route('admin.user.balance.update', $user->id) }}" method="post">
                                @csrf
                            ...
                            <form action="{{ route('admin.user.update', $user->id) }}" method="post">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12 mb-4">
                                        <div class="row">
                                            <div class="col-md-4">
                                                <p for="">{{ __('Withdraw Status') }}</p>
                                                <input type="checkbox" {{ $user->withdrawStatus ? 'checked' : '' }}
                            ...
                            @if ($reference->count() > 0)
                                <ul class="sp-referral">
                                    <li class="single-child root-child">
                                        <p>
                                            <img src="{{ $user->image ? getFile('user', $user->image) : dummyImg() }}">
                                            <span class="mb-0">{{ $user->username }} - (Current User)</span>
                                            {{-- <span class="mb-0">{{ $user->username }} - {{ date_format($user->created_at, 'd-m-Y') }}</span> --}}
                                        </p>
                                        <ul class="sub-child-list step-2">
                                            @foreach ($reference as $user)
                                                <li class="single-child">
                                                    <p>
                                                        <img src="{{ $user->image ? getFile('user', $user->image) : dummyImg() }}">
                                                        <span class="mb-0">{{ $user->username }} - (Join Date: {{ date_format($user->created_at, 'd-m-Y') }})</span>
                                                    </p>
                                                    ...
                                                    <ul class="sub-child-list step-3">
                                                        @foreach ($user->refferals as $ref)
                                                            <li class="single-child">
                                                                <p>
                                                                    <img src="{{ $ref->image ? getFile('user', $ref->image) : dummyImg() }}">
                                                                    <span class="mb-0">{{ $ref->username }} - (Join Date: {{ date_format($ref->created_at, 'd-m-Y') }})</span>
                                                                </p>
                                                                ...
                                                                <ul class="sub-child-list step-4">
                                                                    @foreach ($ref->refferals as $ref2)
                                                                        <li class="single-child">
                                                                            <p>
                                                                                <img
                                                                                    src="{{ $ref2->image ? getFile('user', $ref2->image) : dummyImg() }}">
                                                                                <span class="mb-0">{{ $ref2->username }} - (Join Date: {{ date_format($ref2->created_at, 'd-m-Y') }})</span>
                                                                            </p>
                                                                        </li>
                                                                    @endforeach
                                                                </ul>
